...
public class AdvisorController {
    ...
    protected ModelAndView makeAppoint(Model model, HttpServletRequest request,
            RedirectAttributes rm) throws Exception {
        HttpSession session = request.getSession();
        List<AlertMessage> alerts = (List<AlertMessage>) model.asMap()
                .get("alerts");
        ...
        if (SessionController.isConnected(session)) {
            try {
                ...
                Date appointDate = new SimpleDateFormat("yyyy-MM-dd HH:mm")
                        .parse(completeDate);
                advisorService.makeAppointment(
                        SessionController.getClient(session).getLogin(),
                        appointDate);
                if (alerts != null) {
                    alerts.add(new AlertMessage(AlertEnum.SUCCESS,
                ...
                if (alerts != null) {
                    alerts.add(new AlertMessage(AlertEnum.ERROR,
                            "La date indiquée n'est pas valable"));
                } else {
                    alerts = new ArrayList<AlertMessage>();
                    alerts.add(new AlertMessage(AlertEnum.ERROR,
                            "La date indiquée n'est pas valable"));
                ...
                if (alerts != null) {
                    alerts.add(new AlertMessage(AlertEnum.ERROR,
                            "Votre conseiller n'est pas disponible sur ce créneau"));
                } else {
                    alerts = new ArrayList<AlertMessage>();
                    alerts.add(new AlertMessage(AlertEnum.ERROR,
                            "Votre conseiller n'est pas disponible sur ce créneau"));
    ...
    protected ModelAndView cancelAppoint(Model model,
        ...
        if (SessionController.isConnected(session)) {
            try {
                ...
                if (alerts != null) {
                    alerts.add(new AlertMessage(AlertEnum.ERROR,
                            "Ce rendez-vous n'est pas le votre"));
                } else {
                    alerts = new ArrayList<AlertMessage>();
                    alerts.add(new AlertMessage(AlertEnum.ERROR,
                            "Ce rendez-vous n'est pas le votre"));
