...
import jnpp.controller.client.views.NotifView;
...
public class LinkUserController {
    ...
    protected ModelAndView linkToConnect(Model model,
    ...
    protected ModelAndView linkToSignUp(Model model, HttpServletRequest request)
            throws Exception {
        HttpSession session = request.getSession();
        List<AlertMessage> alerts = (List<AlertMessage>) model.asMap()
                .get("alerts");
        ...
        if (!SessionController.isConnected(session)) {
            return new JNPPModelAndView("signup/signup",
                    ViewInfo.createInfo(session, alerts));
        }
        return new ModelAndView("redirect:/index.htm");
    }
    ...
    protected ModelAndView linkToPersonalSignUp(Model model,
            HttpServletRequest request) throws Exception {
        HttpSession session = request.getSession();
        List<AlertMessage> alerts = (List<AlertMessage>) model.asMap()
                .get("alerts");
        ...
        if (!SessionController.isConnected(session)) {
            ModelAndView view = new JNPPModelAndView("signup/privatesignup",
                    ViewInfo.createInfo(session, alerts));
    ...
    protected ModelAndView linkToProfessionalSignUp(Model model,
            HttpServletRequest request) throws Exception {
        HttpSession session = request.getSession();
        List<AlertMessage> alerts = (List<AlertMessage>) model.asMap()
                .get("alerts");
        ...
        if (!SessionController.isConnected(session)) {
            ModelAndView view = new JNPPModelAndView(
    ...
    protected ModelAndView linkToResume(Model model, HttpServletRequest request,
        ...
        ModelAndView view = new JNPPModelAndView("manageuser/home",
                ViewInfo.createInfo(session, alerts));
        return view;
    }
    ...
    protected ModelAndView linkToUserInfo(Model model,
        ...
        case PRIVATE:
            view = new JNPPModelAndView("manageuser/privateinfo",
                    ViewInfo.createInfo(session, alerts));
            String birthday = new SimpleDateFormat("yyyy-MM-dd")
                    .format(((PrivateDTO) client).getBirthday());
            view.addObject("birthday", birthday);
            break;
        case PROFESIONAL:
            view = new JNPPModelAndView("manageuser/professionalinfo",
                    ViewInfo.createInfo(session, alerts));
            break;
        default:
    ...
    protected ModelAndView linkToPassword(Model model,
            HttpServletRequest request) throws Exception {
        HttpSession session = request.getSession();
        List<AlertMessage> alerts = (List<AlertMessage>) model.asMap()
                .get("alerts");
        ...
        if (!SessionController.isConnected(session)) {
            return new JNPPModelAndView("manageuser/password",
                    ViewInfo.createInfo(session, alerts));
        }
        return new ModelAndView("redirect:/index.htm");
    }
     ...
     *         les particuliers si l'utilisateur n'est pas connecté, redirection
     *         vers l'index sinon
     * @throws Exception Exception non controllees.
     */
    @RequestMapping(value = "privatepassword", method = RequestMethod.GET)
    protected ModelAndView linkToPrivatePassword(Model model,
            HttpServletRequest request) throws Exception {
        HttpSession session = request.getSession();
        List<AlertMessage> alerts = (List<AlertMessage>) model.asMap()
                .get("alerts");
    ...
    protected ModelAndView linkToProfessionalPassword(Model model,
            HttpServletRequest request) throws Exception {
        HttpSession session = request.getSession();
        List<AlertMessage> alerts = (List<AlertMessage>) model.asMap()
                .get("alerts");
    ...
    protected ModelAndView linkToNotifs(Model model, HttpServletRequest request,
            RedirectAttributes rm) throws Exception {
        HttpSession session = request.getSession();
        List<AlertMessage> alerts = (List<AlertMessage>) model.asMap()
                .get("alerts");
        ...
        if (SessionController.isConnected(session)) {
            try {
                List<NotificationDTO> notifs = notifService
                        .receiveNotifications(SessionController
                                .getClient(session).getLogin());
                List<NotifView> notifsView = new ArrayList<NotifView>();
                for (NotificationDTO notif : notifs) {
                    notifsView.add(new NotifView(notif));
                }
                notifService.seeAllNotications(
                        SessionController.getClient(session).getLogin());
                SessionController.setHasNotif(session, false);
                ModelAndView view = new JNPPModelAndView("manageuser/notifs",
                        ViewInfo.createInfo(session, alerts));
