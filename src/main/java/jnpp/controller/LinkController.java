...
/**
 * Classe contrôlant les différents liens qui ne requièrent pas de traitement
 * particulier
 */
@Controller
public class LinkController {
    ...
    protected ModelAndView linkToIndex(Model model, HttpServletRequest request,
        ...
        return new JNPPModelAndView("index",
    ...
    protected ModelAndView linkToSignUpSuccess(Model model,
        ...
        if (!SessionController.isConnected(session)) {
            return new JNPPModelAndView("signup/signupsuccess",
                    ViewInfo.createInfo(session, alerts));
        }
    ...
    protected ModelAndView linkToPasswordSuccess(Model model,
            HttpServletRequest request) throws Exception {
        HttpSession session = request.getSession();
        List<AlertMessage> alerts = (List<AlertMessage>) model.asMap()
                .get("alerts");
        ...
        if (!SessionController.isConnected(session)) {
            return new JNPPModelAndView("manageuser/passwordsuccess",
                    ViewInfo.createInfo(session, alerts));
        }
        return new ModelAndView("redirect:/index.htm");
    }
