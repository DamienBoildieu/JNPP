...
@Entity
@NamedQueries({
        @NamedQuery(name = "find_all_advisor", query = "SELECT a FROM AdvisorEntity a"),
        @NamedQuery(name = "find_advisor_by_identity", query = "SELECT a FROM AdvisorEntity a "
                + "WHERE a.identity.firstname = :firstname "
                + "  AND a.identity.lastname = :lastname"),
        @NamedQuery(name = "find_advisor_clients_by_id", query = "SELECT a_clients FROM AdvisorEntity a INNER JOIN a.clients a_clients WHERE a.id = :id") })
public class AdvisorEntity implements Serializable {
    ...
    public AdvisorDTO toDTO() {
        return new AdvisorDTO(identity.toDTO(), email, phone,
                officeAdress.toDTO());
    }
