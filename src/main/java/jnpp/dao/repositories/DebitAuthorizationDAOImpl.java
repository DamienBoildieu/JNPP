...
@Repository
public class DebitAuthorizationDAOImpl
        extends GenericDAOImpl<DebitAuthorizationEntity>
        implements DebitAuthorizationDAO {

    @Transactional(readOnly = true)
    @Override
    public DebitAuthorizationEntity findByRibFromRibTo(String ribFrom,
            String ribTo) {
        Query query = getEm().createNamedQuery(
                "find_debit_authorization_by_rib_from_rib_to");
    ...
    public List<DebitAuthorizationEntity> findAllByLogin(String login) {
        Query query = getEm()
                .createNamedQuery("find_all_debit_authorization_by_login");
    ...
    @Transactional(readOnly = true)
    @Override
    public List<DebitAuthorizationEntity> findAllByLoginRibFrom(String login,
            String rib) {
        Query query = getEm().createNamedQuery(
                "find_all_debit_authorization_by_login_rib_from");
        query.setParameter("login", login);
